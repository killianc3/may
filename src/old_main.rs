...
fn main() {
    ...
    let trackbar = unsafe { CreateWindowExW(
            ...
            null_mut(),
            WS_CHILD | WS_VISIBLE | 0x0020 as u32,
            200,
            0,
            200,
            40,
                ...
                NM_CUSTOMDRAW => {
                    ...
                    match (*draw_st).dwDrawStage {
                        ...
                        CDDS_ITEMPREPAINT => match (*draw_st).dwItemSpec {
                            1 => return CDRF_SKIPDEFAULT,
                            3 => {
                                let rc = RECT::default();
                                let _ = get_client_rect((*nmhdr).hwndFrom, &rc).unwrap();
                                //let _ = fill_rect((*draw_st).hdc, &rc, CreateSolidBrush(PLAY_COLORREF));
                                return CDRF_SKIPDEFAULT
                            }
                            2 => {
                                println!("test");
                                let dc = GetDC((*nmhdr).hwndFrom);
                                let rc = RECT::default();
                                get_client_rect((*nmhdr).hwndFrom, &rc);
                                //let _ = fill_rect((*draw_st).hdc, &rc, CreateSolidBrush(PLAY_COLORREF));

                                let rc2 = &(*draw_st).rc;
                                let thumb = RECT { left: rc.left, top: rc2.top, right: rc2.right, bottom: rc2.bottom };
                                fill_rect(dc, &thumb, CreateSolidBrush(0x000FFFFF as COLORREF));
                                return CDRF_SKIPDEFAULT
                            }
                            _ => println!("{}", (*draw_st).dwItemSpec as DWORD),
                        },
